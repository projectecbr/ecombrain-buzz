...
test("parsePromptText wraps header-less free text in a single Prompt section", () => {
  ...
  // section is a legacy "Buzz event", there is no event content to surface, so
  // userText is empty and the branded title falls through to "Teams event".
  ...
  assert.equal(result.userTitle, "Teams event");
...
test("parsePromptText defaults the branded title to 'Teams event' when no kind is present", () => {
